...
app.get('/', async (c: any) => {
  ...
  const filters = {
    ...
    sort_by: (c.req.query('sort') as any) || 'created_at',
...
app.get('/:id', async (c: any) => {
  ...
  if (!result.success || (result.data as any)?.status !== 'completed') return c.json({ success: false, error: 'Order not found' }, 404)
...
app.post('/', validateRequest({
  ...
  const updateRes = await service.updateOrderStatus(orderId, tenantId, 'completed')
  if (!updateRes.success) return c.json({ success: false, error: 'Failed to complete order' }, 400)
