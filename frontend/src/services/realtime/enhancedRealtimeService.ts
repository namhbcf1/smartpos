...
class EnhancedRealtimeService extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    try {
      // Skip WebSocket (not supported on Cloudflare Workers), try other methods
      const success = await this.tryServerSentEvents() || 
  ...
  private async tryServerSentEvents(): Promise<boolean> {
    try {
      const sseUrl = `${import.meta.env.VITE_API_BASE_URL || 'https://namhbcf-api.bangachieu2.workers.dev'}${API_ENDPOINTS.REALTIME.STREAM}`;
  ...
  private getWebSocketUrl(): string {
    const apiUrl = import.meta.env.VITE_API_BASE_URL || 'https://namhbcf-api.bangachieu2.workers.dev';
