...
const WarrantyNotifications: React.FC = () => {
  ...
  // Load notification data from D1 Cloudflare
  useEffect(() => {
    const loadNotificationData = async () => {
      try {
        const [rulesResponse, templatesResponse, historyResponse] = await Promise.all([
          apiClient.get('/warranty/notification-rules'),
          apiClient.get('/warranty/notification-templates'),
          apiClient.get('/warranty/notification-history')
        ]);
