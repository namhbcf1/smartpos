...
const Categories: React.FC = () => {
  ...
  const queryParams = useMemo(() => ({
    ...
    is_active: statusFilter === '' ? undefined : statusFilter === 'true' ? 'true' : 'false',
    sortBy: 'name',
  ...
  const handleSubmitForm = async () => {
    if (!formData.name.trim()) {
      enqueueSnackbar('Tên danh mục không được để trống', { variant: 'error' });
      return;
    }
    ...
    if (editingCategory) {
      ...
      if (response) {
        ...
        enqueueSnackbar('Không thể cập nhật danh mục', { variant: 'error' });
      }
    } else {
      ...
      if (response) {
        ...
        enqueueSnackbar('Không thể tạo danh mục', { variant: 'error' });
      }
    }
