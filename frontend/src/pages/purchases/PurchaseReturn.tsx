...
export default function PurchaseReturn() {
  ...
  const submit = async () => {
    ...
    setLoading(true)
    try {
      await apiClient.post(`/purchase-orders/${poId}/return`, { reason })
      navigate('/purchases')
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Trả hàng PO</h1>
      ...
      <div className="bg-base-100 rounded-lg shadow p-4 max-w-xl">
        ...
        <div className="mb-4">
          <label className="block text-sm mb-1">Lý do trả hàng</label>
          <textarea className="textarea textarea-bordered w-full" rows={3} value={reason} onChange={(e) => setReason(e.target.value)} />
        </div>
