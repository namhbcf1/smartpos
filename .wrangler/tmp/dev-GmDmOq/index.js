...
// .wrangler/tmp/bundle-aQZaKY/checked-fetch.js
...
// .wrangler/tmp/bundle-aQZaKY/strip-cf-connecting-ip-header.js
...
function validatePasswordStrength(password) {
  ...
  if (password.length < 4) {
    errors.push("M\u1EADt kh\u1EA9u ph\u1EA3i c\xF3 \xEDt nh\u1EA5t 4 k\xFD t\u1EF1");
  }
  return {
...
// .wrangler/tmp/bundle-aQZaKY/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// .wrangler/tmp/bundle-aQZaKY/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
  ...
  route(path, app26) {
    ...
    app26.routes.map((r) => {
      ...
      if (app26.errorHandler === errorHandler) {
        ...
        handler = /* @__PURE__ */ __name(async (c, next) => (await compose([], app26.errorHandler)(c, () => r.handler(c, next))).res, "handler");
...
function buildMatcherFromPreprocessedRoutes(routes) {
  ...
  for (let i = 0, j = -1, len = routesWithStaticPathFlag.length; i < len; i++) {
    const [pathErrorCheckOnly, path, handlers7] = routesWithStaticPathFlag[i];
    if (pathErrorCheckOnly) {
      staticMap[path] = [handlers7.map(([h]) => [h, {}]), emptyParam];
    } else {
      j++;
    }
    let paramAssoc;
    ...
    handlerData[j] = handlers7.map(([h, paramCount]) => {
      const paramIndexMap = {};
      paramCount -= 1;
...
async function loginHandler(c) {
  try {
    ...
    const authResponse = {
      ...
      // 24 hours
      session_id: "simple-session-" + user.id
    };
    return c.json({
      success: true,
      data: authResponse,
...
async function registerHandler(c) {
  try {
    ...
    if (role === "affiliate" || role === "sales_agent") {
      dbRole = "inventory";
      console.log(`\u{1F504} Mapping role '${role}' to '${dbRole}' for database compatibility`);
    }
    if (password.length < 3) {
      return c.json({
        success: false,
        data: null,
        message: "M\u1EADt kh\u1EA9u ph\u1EA3i c\xF3 \xEDt nh\u1EA5t 3 k\xFD t\u1EF1"
      }, 400);
    }
    ...
    if (existingUserByUsername) {
      return c.json({
        success: false,
        data: null,
        message: "T\xEAn \u0111\u0103ng nh\u1EADp \u0111\xE3 \u0111\u01B0\u1EE3c s\u1EED d\u1EE5ng b\u1EDFi t\xE0i kho\u1EA3n kh\xE1c"
      }, 400);
    }
    ...
    if (existingUserByEmail) {
      ...
      if (!updatedUser) {
        return c.json({
          success: false,
          data: null,
          message: "L\u1ED7i c\u1EADp nh\u1EADt t\xE0i kho\u1EA3n"
        }, 500);
      }
      const user2 = {
        id: updatedUser.id,
        ...
        role,
        // Return the original role, not the mapped dbRole
      ...
      return c.json({
        success: true,
        data: user2,
        message: "C\u1EADp nh\u1EADt t\xE0i kho\u1EA3n th\xE0nh c\xF4ng"
      }, 200);
    }
  ...
  try {
    ...
    if (new_password.length < 3) {
      return c.json({
        success: false,
        data: null,
        message: "M\u1EADt kh\u1EA9u m\u1EDBi ph\u1EA3i c\xF3 \xEDt nh\u1EA5t 3 k\xFD t\u1EF1"
      }, 400);
    }
    const user = await c.env.DB.prepare(`
...
app.post("/create-users-from-employees", async (c) => {
  try {
    ...
    for (const employee of employees.results) {
      try {
        let username = employee.full_name.toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9]/g, "");
        ...
        else if (employee.role === "affiliate")
          userRole = "affiliate";
        else if (employee.role === "cashier")
          userRole = "cashier";
        else if (employee.role === "inventory")
          userRole = "inventory";
        else
          userRole = "cashier";
        const passwordHash = await hashPassword2(defaultPassword);
        const result = await c.env.DB.prepare(`
    ...
    return c.json({
      ...
      data: {
        ...
        users: createdUsers,
        login_info: createdUsers.filter((u) => u.status === "created").map((u) => ({
...
app.post("/update-user-role", async (c) => {
  try {
    ...
    console.log(`\u{1F527} Updating user ${username} role to ${role}...`);
    ...
    console.log("\u{1F464} Updating role using D1 batch API...");
    try {
...
// src/routes/products/handlers.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// src/routes/products/service.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();

// src/routes/products/database.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var ProductDatabase = class {
  constructor(env) {
    this.env = env;
  }
  ...
  async createSampleData() {
    try {
      const categoryCount = await this.env.DB.prepare("SELECT COUNT(*) as count FROM categories WHERE deleted_at IS NULL").first();
      if (!categoryCount || categoryCount.count === 0) {
        console.log("Creating sample categories...");
        const categories = [
          { name: "M\xE1y t\xEDnh \u0111\u1EC3 b\xE0n", description: "M\xE1y t\xEDnh \u0111\u1EC3 b\xE0n v\xE0 workstation", slug: "may-tinh-de-ban" },
          { name: "Laptop", description: "M\xE1y t\xEDnh x\xE1ch tay c\xE1c lo\u1EA1i", slug: "laptop" },
          { name: "Linh ki\u1EC7n", description: "Linh ki\u1EC7n m\xE1y t\xEDnh", slug: "linh-kien" },
          { name: "Ph\u1EE5 ki\u1EC7n", description: "Ph\u1EE5 ki\u1EC7n m\xE1y t\xEDnh", slug: "phu-kien" },
          { name: "Gaming", description: "S\u1EA3n ph\u1EA9m gaming chuy\xEAn d\u1EE5ng", slug: "gaming" }
        ];
      ...
      const productCount = await this.env.DB.prepare("SELECT COUNT(*) as count FROM products WHERE deleted_at IS NULL").first();
      if (!productCount || productCount.count === 0) {
        console.log("Creating sample products...");
        const products = [
          {
            name: "PC Gaming RTX 4060",
            description: "M\xE1y t\xEDnh gaming v\u1EDBi card RTX 4060, CPU i5-12400F, RAM 16GB, SSD 500GB",
            sku: "PC-RTX4060-001",
          ...
          },
          {
            ...
            description: "B\u1ED9 vi x\u1EED l\xFD Intel Core i5-12400F, 6 cores 12 threads, 2.5GHz base clock",
            sku: "CPU-I5-12400F",
  ...
  async getStats() {
    try {
      ...
      return {
        ...
        suppliers_count: 0
        // Will be implemented when suppliers module is ready
      };
    } catch (error) {
...
__name(ProductDatabase, "ProductDatabase");
...
var ProductService = class {
  ...
  async getProducts(params2) {
    try {
      ...
      const query = `
        ...
        LIMIT ? OFFSET ?
      `;
      const products = await this.env.DB.prepare(query).bind(...bindings, limit, offset).all();
      const countQuery = `
  ...
  async getProductById(id) {
    try {
      console.log("\u{1F50D} Getting product by ID:", id);
      console.log("\u{1F4CA} Executing database query...");
      ...
      `).bind(id).first();
      console.log("\u{1F4E6} Database result:", product);
      return product || null;
    } catch (error) {
  ...
  async createProduct(data, userId) {
    try {
      ...
      const category = await this.env.DB.prepare("SELECT id FROM categories WHERE id = ? AND deleted_at IS NULL").bind(data.category_id).first();
      if (!category) {
  ...
  async updateStock(productId, quantity, movementType, referenceType, referenceId, notes, userId) {
    try {
...
var ProductHandlers = class {
  ...
  async getProducts(c) {
    try {
      ...
      const params2 = {
        ...
        search: query.search,
        category_id: query.category_id ? parseInt(query.category_id) : void 0,
        supplier_id: query.supplier_id ? parseInt(query.supplier_id) : void 0,
        brand: query.brand,
        is_active: query.is_active ? query.is_active === "true" : void 0,
        is_featured: query.is_featured ? query.is_featured === "true" : void 0,
        in_stock_only: query.in_stock_only === "true",
        low_stock_only: query.low_stock_only === "true",
        price_min: query.price_min ? parseFloat(query.price_min) : void 0,
        price_max: query.price_max ? parseFloat(query.price_max) : void 0,
      ...
      const products = await c.env.DB.prepare(`
        ...
        ORDER BY created_at DESC
        LIMIT ? OFFSET ?
      `).bind(params2.limit || 20, ((params2.page || 1) - 1) * (params2.limit || 20)).all();
      ...
      const response = {
        ...
        pagination: {
          ...
          total,
          totalPages: Math.ceil(total / (params2.limit || 20))
        }
      };
  ...
  async getProductById(c) {
    try {
      ...
      console.log("\u{1F50D} Handler - Getting product ID:", id);
      const product = await c.env.DB.prepare(`
        SELECT * FROM products WHERE id = ?
      `).bind(id).first();
      console.log("\u{1F4E6} Handler database result:", product);
      if (!product) {
        return c.json({
          success: false,
  ...
  async createProduct(c) {
    try {
      ...
      if (!data.name || !data.sku || !data.category_id || data.price === void 0 || data.cost_price === void 0) {
        return c.json({
          success: false,
  ...
  async updateStock(c) {
    try {
      ...
      if (quantity === void 0 || !movement_type) {
        return c.json({
          success: false,
...
__name(ProductHandlers, "ProductHandlers");
...
app2.get("/barcode/:barcode", (c) => handlers.getProductByBarcode(c));
app2.get("/sku/:sku", (c) => handlers.getProductBySku(c));
app2.get("/", (c) => handlers.getProducts(c));
app2.get("/stats", (c) => handlers.getStats(c));
app2.get("/detail/:id", async (c) => {
  const id = parseInt(c.req.param("id"));
  if (isNaN(id)) {
    return c.json({
      success: false,
...
app2.get("/:id", async (c) => {
  try {
    ...
    console.log("\u{1F50D} Getting product ID:", id);
    const product = await c.env.DB.prepare(`
      SELECT
        id, name, sku, barcode, category_id, price, cost_price,
        tax_rate, stock_quantity, min_stock_level, max_stock_level,
        reorder_point, reorder_quantity, is_active, image_url,
        brand, description, discount_eligible, model, unit, weight,
        dimensions, color, size, supplier_id, supplier_sku, supplier_price,
        warranty_period_months, warranty_type, warranty_terms,
        image_urls, video_url, meta_title, meta_description, tags,
        is_featured, is_digital, requires_shipping, track_quantity,
    ...
    if (product.category_id) {
      try {
        ...
        console.log("Category query failed, continuing without category name");
      }
    }
    ...
    if (product.supplier_id) {
      try {
        ...
        `).bind(product.supplier_id).first();
        supplierName = supplier?.name || null;
      } catch (err) {
        console.log("Supplier query failed, continuing without supplier name");
      }
    }
    const response = {
      // Basic info
      ...
      discountEligible: Boolean(product.discount_eligible),
      // Inventory
      stockQuantity: parseInt(product.stock_quantity || 0),
      stockAlertThreshold: parseInt(product.min_stock_level || 0),
      minStockLevel: parseInt(product.min_stock_level || 0),
      maxStockLevel: product.max_stock_level ? parseInt(product.max_stock_level) : null,
      reorderPoint: product.reorder_point ? parseInt(product.reorder_point) : null,
      reorderQuantity: product.reorder_quantity ? parseInt(product.reorder_quantity) : null,
      // Product details
      brand: product.brand,
      model: product.model,
      ...
      supplierPrice: product.supplier_price ? parseFloat(product.supplier_price) : null,
      // Warranty
      warrantyPeriodMonths: parseInt(product.warranty_period_months || 12),
      warrantyType: product.warranty_type || "manufacturer",
      ...
      trackQuantity: Boolean(product.track_quantity),
      // Audit
    ...
    console.log("\u2705 Returning product:", response);
    return c.json({
      success: true,
      data: response
    });
...
app2.post("/", authorize(["admin", "manager"]), (c) => handlers.createProduct(c));
app2.put("/:id", authorize(["admin", "manager"]), (c) => handlers.updateProduct(c));
app2.delete("/:id", authorize(["admin"]), (c) => handlers.deleteProduct(c));
app2.post("/:id/stock", authorize(["admin", "manager", "cashier"]), (c) => handlers.updateStock(c));
...
var userCreateSchema = external_exports.object({
  ...
  role: external_exports.enum(["admin", "manager", "cashier", "inventory", "sales_agent", "affiliate"]),
...
app4.use("*", async (c, next) => {
  try {
    if (!handlers2) {
      ...
      handlers2 = new SalesHandlers(c.env);
...
app4.get("/summary", (c) => handlers2.getSalesSummary(c));
app4.get("/stats", (c) => handlers2.getStats(c));
app4.get("/today", (c) => handlers2.getTodaysSummary(c));
app4.get("/recent", (c) => handlers2.getRecentSales(c));
app4.get("/", (c) => handlers2.getSales(c));
app4.get("/:id", (c) => handlers2.getSaleById(c));
app4.post("/", (c) => handlers2.createSale(c));
app4.post("/quick", (c) => handlers2.createQuickSale(c));
app4.put("/:id", (c) => handlers2.updateSale(c));
app4.post("/:id/print-receipt", (c) => handlers2.printReceipt(c));
        ...
        const roles = [
          ...
          },
          {
            ...
            permissions: [
              "products.view",
              "products.update",
              // Thu ngân có thể sửa đổi sản phẩm
              "sales.view",
              "sales.create",
              "sales.update",
              "returns.view",
          ...
          },
          {
            ...
            permissions: [
              "products.view",
              // Chỉ xem sản phẩm, không sửa đổi
              "sales.view",
              "sales.create",
              "sales.update",
              "returns.view",
...
app5.use("*", async (c, next) => {
  if (!handlers3) {
    handlers3 = new UserHandlers(c.env);
    await handlers3.initialize();
  }
  await next();
});
app5.use("*", authenticate);
app5.get("/me", (c) => handlers3.getCurrentUser(c));
app5.put("/me", (c) => handlers3.updateCurrentUser(c));
app5.get("/stats", authorize(["admin", "manager"]), (c) => handlers3.getStats(c));
app5.get("/", authorize(["admin", "manager"]), (c) => handlers3.getUsers(c));
app5.get("/username/:username", authorize(["admin", "manager"]), (c) => handlers3.getUserByUsername(c));
app5.get("/employee/:employeeId", authorize(["admin", "manager"]), (c) => handlers3.getUserByEmployeeId(c));
app5.get("/:id", authorize(["admin", "manager"]), (c) => handlers3.getUserById(c));
app5.post("/", authorize(["admin"]), (c) => handlers3.createUser(c));
app5.put("/:id", authorize(["admin"]), (c) => handlers3.updateUser(c));
app5.delete("/:id", authorize(["admin"]), (c) => handlers3.deleteUser(c));
...
reports.get("/definitions", async (c) => {
  try {
    const definitions = [
      {
        ...
        description: "Ph\xE2n t\xEDch doanh thu theo th\u1EDDi gian v\u1EDBi AI insights v\xE0 d\u1EF1 b\xE1o xu h\u01B0\u1EDBng",
        category: "sales",
        icon: "\u{1F4B0}",
      ...
      },
      {
        ...
        name: "Ph\xE2n t\xEDch t\xE0i ch\xEDnh",
        description: "B\xE1o c\xE1o t\xE0i ch\xEDnh chi ti\u1EBFt v\u1EDBi d\u1EF1 b\xE1o AI v\xE0 ph\xE2n t\xEDch r\u1EE7i ro",
        category: "financial",
        icon: "\u{1F4CA}",
      ...
      },
      {
        ...
        name: "Th\xF4ng minh kho h\xE0ng",
        description: "AI ph\xE2n t\xEDch t\u1ED3n kho, d\u1EF1 b\xE1o nhu c\u1EA7u v\xE0 t\u1ED1i \u01B0u h\xF3a",
        category: "inventory",
        icon: "\u{1F4E6}",
      ...
      },
      {
        ...
        description: "Ph\xE2n t\xEDch h\xE0nh vi kh\xE1ch h\xE0ng v\u1EDBi machine learning v\xE0 personalization",
        category: "customer",
        icon: "\u{1F465}",
      ...
      },
      {
        ...
        name: "Hi\u1EC7u su\u1EA5t b\xE1n h\xE0ng",
        description: "Ph\xE2n t\xEDch hi\u1EC7u su\u1EA5t b\xE1n h\xE0ng theo s\u1EA3n ph\u1EA9m, nh\xE2n vi\xEAn v\u1EDBi AI coaching",
        category: "sales",
        icon: "\u{1F3AF}",
      ...
      },
      {
        ...
        name: "D\u1EF1 b\xE1o th\xF4ng minh",
        description: "Machine learning d\u1EF1 b\xE1o doanh thu, xu h\u01B0\u1EDBng v\xE0 c\u01A1 h\u1ED9i kinh doanh",
        category: "analytics",
        icon: "\u{1F9E0}",
    ...
    return c.json({
      success: true,
      message: "Danh s\xE1ch b\xE1o c\xE1o th\xF4ng minh",
      data: {
        reports: definitions,
...
reports.get("/revenue", async (c) => {
  try {
    const dateFrom = c.req.query("dateFrom") || new Date(Date.now() - 30 * 24 * 60 * 60 * 1e3).toISOString().split("T")[0];
    const dateTo = c.req.query("dateTo") || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      const orders = Math.floor(revenue / (2e6 + Math.random() * 1e6));
    ...
    const insights = [
      {
        type: "trend",
        title: "Xu h\u01B0\u1EDBng doanh thu t\xEDch c\u1EF1c",
      ...
      {
        type: "pattern",
        title: "Ph\xE2n t\xEDch theo ng\xE0y trong tu\u1EA7n",
        description: "Cu\u1ED1i tu\u1EA7n c\xF3 doanh thu cao h\u01A1n 40% so v\u1EDBi ng\xE0y th\u01B0\u1EDDng",
        confidence: 0.94,
        impact: "medium",
        recommendation: "T\u1ED1i \u01B0u staffing: t\u0103ng nh\xE2n s\u1EF1 cu\u1ED1i tu\u1EA7n",
        potentialValue: Math.floor(totalRevenue * 0.08)
      }
    ];
    return c.json({
      success: true,
      message: "B\xE1o c\xE1o doanh thu th\xF4ng minh",
      data: {
        summary: {
          totalRevenue,
          totalOrders,
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA1o b\xE1o c\xE1o doanh thu",
      error: error.message
    }, 500);
  }
});
reports.get("/financial", async (c) => {
  try {
    ...
    for (let i = periods - 1; i >= 0; i--) {
      ...
      const revenue = 8e8 + Math.random() * 4e8;
      const expenses = revenue * (0.6 + Math.random() * 0.2);
      const profit = revenue - expenses;
      const profitMargin = profit / revenue * 100;
      const cashFlow = profit + (Math.random() - 0.5) * 1e8;
      financialData.push({
        period: period === "monthly" ? date.toLocaleDateString("vi-VN", { year: "numeric", month: "long" }) : period === "quarterly" ? `Q${Math.floor(date.getMonth() / 3) + 1} ${date.getFullYear()}` : `Tu\u1EA7n ${Math.ceil((Date.now() - date.getTime()) / (7 * 24 * 60 * 60 * 1e3))}`,
        revenue,
        expenses,
        profit,
        profitMargin,
    ...
    const insights = [
      {
        ...
        description: `T\u1EF7 su\u1EA5t l\u1EE3i nhu\u1EADn duy tr\xEC \u1EDF m\u1EE9c ${latestData.profitMargin.toFixed(1)}%`,
        confidence: 0.91,
        recommendation: "Duy tr\xEC chi\u1EBFn l\u01B0\u1EE3c pricing hi\u1EC7n t\u1EA1i"
      },
      {
        type: "opportunity",
        title: "C\u01A1 h\u1ED9i t\u1ED1i \u01B0u cash flow",
        description: "Cash flow c\xF3 th\u1EC3 c\u1EA3i thi\u1EC7n 15% b\u1EB1ng c\xE1ch t\u1ED1i \u01B0u chu k\u1EF3 thanh to\xE1n",
        confidence: 0.84,
    ...
    return c.json({
      success: true,
      message: "B\xE1o c\xE1o t\xE0i ch\xEDnh th\xF4ng minh",
      data: {
        summary: {
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA1o b\xE1o c\xE1o t\xE0i ch\xEDnh",
      error: error.message
    }, 500);
  }
});
...
init_strip_cf_connecting_ip_header();
var app7 = new Hono2();
app7.get("/overview", authenticate, async (c) => {
  try {
...
app7.get("/ranking", authenticate, async (c) => {
  try {
...
app7.get("/analytics/:id", authenticate, async (c) => {
  try {
...
app7.get("/performance", authenticate, async (c) => {
  try {
...
app7.get("/:id", authenticate, async (c) => {
  try {
    const storeId = c.req.param("id");
...
app7.post("/", authenticate, async (c) => {
  try {
    const body = await c.req.json();
...
app7.put("/:id", authenticate, async (c) => {
  try {
    const storeId = c.req.param("id");
    const body = await c.req.json();
...
app7.delete("/:id", authenticate, async (c) => {
  try {
    const storeId = c.req.param("id");
    const existingStore = await c.env.DB.prepare("SELECT id FROM stores WHERE id = ?").bind(storeId).first();
    if (!existingStore) {
...
app8.use("*", async (c, next) => {
  if (!handlers4) {
    handlers4 = new InventoryHandlers(c.env);
...
app8.get("/locations", (c) => handlers4.getLocations(c));
app8.get("/suppliers", (c) => handlers4.getSuppliers(c));
app8.get("/low-stock", (c) => handlers4.getLowStockItems(c));
app8.get("/out-of-stock", (c) => handlers4.getOutOfStockItems(c));
app8.get("/", (c) => handlers4.getInventoryItems(c));
app8.get("/:id", (c) => handlers4.getInventoryItemById(c));
app8.post("/", authorize(["admin", "manager"]), (c) => handlers4.createInventoryItem(c));
app8.put("/:id", authorize(["admin", "manager"]), (c) => handlers4.updateInventoryItem(c));
app8.post("/bulk-update", authorize(["admin", "manager"]), (c) => handlers4.bulkUpdateInventory(c));
var inventory_default = app8;
...
app9.use("*", async (c, next) => {
  if (!handlers5) {
    handlers5 = new ReturnsHandlers(c.env);
    await handlers5.initialize();
  }
  await next();
});
app9.use("*", authenticate);
app9.get("/stats", authorize(["admin", "manager"]), (c) => handlers5.getStats(c));
app9.get("/recent", (c) => handlers5.getRecentReturns(c));
app9.get("/pending", (c) => handlers5.getPendingReturns(c));
app9.get("/", (c) => handlers5.getReturns(c));
app9.get("/:id", (c) => handlers5.getReturnById(c));
app9.post("/", (c) => handlers5.createReturn(c));
app9.put("/:id", (c) => handlers5.updateReturn(c));
app9.post("/:id/approve", authorize(["admin", "manager"]), (c) => handlers5.approveReturn(c));
app9.post("/:id/reject", authorize(["admin", "manager"]), (c) => handlers5.rejectReturn(c));
app9.post("/:id/complete", (c) => handlers5.completeReturn(c));
var returns_default = app9;
...
app10.use("*", async (c, next) => {
  if (!handlers6) {
    handlers6 = new CustomersHandlers(c.env);
    await handlers6.initialize();
  }
  await next();
});
app10.get("/stats", (c) => handlers6.getStats(c));
app10.get("/cities", (c) => handlers6.getCities(c));
app10.get("/search", (c) => handlers6.searchCustomers(c));
app10.get("/vip", (c) => handlers6.getVIPCustomers(c));
app10.get("/recent", (c) => handlers6.getRecentCustomers(c));
...
app10.get("/", (c) => handlers6.getCustomers(c));
app10.get("/:id", (c) => handlers6.getCustomerById(c));
app10.post("/", (c) => handlers6.createCustomer(c));
app10.put("/:id", (c) => handlers6.updateCustomer(c));
app10.post("/:id/loyalty-points", (c) => handlers6.addLoyaltyPoints(c));
var customers_default = app10;
...
app11.get("/", async (c) => {
  try {
    ...
    let whereClause2 = "WHERE status != 'deleted'";
    const params2 = [];
    if (search) {
...
app11.get("/schema-debug", async (c) => {
  try {
...
app11.get("/active", async (c) => {
  try {
    await initializeEmployeeTables(c.env);
    const role = c.req.query("role") || "";
    let whereClause2 = "WHERE status = ?";
...
app12.post("/create-samples", async (c) => {
  try {
    ...
    return c.json({
      success: true,
      message: "T\u1EA1o nh\xE0 cung c\u1EA5p m\u1EABu th\xE0nh c\xF4ng",
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA1o nh\xE0 cung c\u1EA5p m\u1EABu",
      error: error.message
    }, 500);
  }
});
...
app12.post("/", authenticate, async (c) => {
  try {
...
app12.put("/:id", authenticate, validateBody(supplierUpdateSchema), async (c) => {
  try {
    const id = parseInt(c.req.param("id"));
...
app12.get("/:id/performance", async (c) => {
  try {
    ...
    const performance = {
      ...
      compliance_score: 92,
      notes: [
        "Giao h\xE0ng \u0111\xFAng h\u1EA1n, ch\u1EA5t l\u01B0\u1EE3ng t\u1ED1t",
        "Gi\xE1 c\u1EA3 c\u1EA1nh tranh",
        "H\u1ED7 tr\u1EE3 k\u1EF9 thu\u1EADt t\u1ED1t"
      ],
      trends: {
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y hi\u1EC7u su\u1EA5t nh\xE0 cung c\u1EA5p"
    }, 500);
  }
});
app12.post("/:id/ratings", async (c) => {
  try {
    const supplierId = parseInt(c.req.param("id"));
    ...
    return c.json({
      success: true,
      message: "\u0110\xE1nh gi\xE1 \u0111\xE3 \u0111\u01B0\u1EE3c l\u01B0u"
    });
  } catch (error) {
    console.error("Error adding supplier rating:", error);
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u01B0u \u0111\xE1nh gi\xE1"
    }, 500);
  }
});
...
var app14 = new Hono2();
app14.get("/ws", (c) => {
  return c.json({ message: "WebSocket endpoint temporarily disabled" });
});
var websocket_default = app14;
...
app15.get("/", authenticate, async (c) => {
  try {
    const env = c.env;
    const query = c.req.query();
...
app15.get("/stats", authenticate, async (c) => {
  try {
    const env = c.env;
...
app15.get("/:id", authenticate, async (c) => {
  try {
    const env = c.env;
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
...
app15.get("/search/:serial", authenticate, async (c) => {
  try {
    const env = c.env;
...
app15.post("/", authenticate, authorize(["admin", "manager", "inventory"]), validate(serialNumberCreateSchema), auditLogger, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
...
app15.put("/:id", authenticate, authorize(["admin", "manager", "inventory"]), validate(serialNumberUpdateSchema), auditLogger, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
    const id = parseInt(c.req.param("id"));
...
app15.delete("/:id", authenticate, authorize(["admin", "manager"]), auditLogger, async (c) => {
  try {
    const env = c.env;
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
...
app15.post("/bulk", authenticate, authorize(["admin", "manager", "inventory"]), auditLogger, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
    const body = await c.req.json();
...
var bulkImportSchema = external_exports.object({
  serial_numbers: external_exports.array(external_exports.object({
    serial_number: external_exports.string(),
    product_id: external_exports.number().optional(),
    product_name: external_exports.string().optional(),
    product_sku: external_exports.string().optional(),
    location: external_exports.string().optional(),
    notes: external_exports.string().optional()
  })),
  stock_in_id: external_exports.string().optional()
});
app15.post(
  "/bulk-import",
  authenticate,
  ...
  async (c) => {
    try {
        ...
        try {
          ...
          if (existing) {
            ...
            results.errors.push({
              ...
              error: "Serial number \u0111\xE3 t\u1ED3n t\u1EA1i trong h\u1EC7 th\u1ED1ng"
            });
            continue;
          }
          if (!isValidSerialFormat(serialData.serial_number)) {
            ...
            results.errors.push({
              serial_number: serialData.serial_number,
              error: "\u0110\u1ECBnh d\u1EA1ng serial number kh\xF4ng h\u1EE3p l\u1EC7"
            });
            continue;
          }
          await env.DB.prepare(`
          ...
          results.errors.push({
            serial_number: serialData.serial_number,
            error: error.message || "L\u1ED7i kh\xF4ng x\xE1c \u0111\u1ECBnh"
          });
        }
      }
      ...
      return c.json({
        success: true,
        data: results,
        message: `Import ho\xE0n t\u1EA5t: ${results.successful} th\xE0nh c\xF4ng, ${results.failed} l\u1ED7i, ${results.duplicates} tr\xF9ng l\u1EB7p`
      });
    } catch (error) {
      console.error("Error bulk importing serial numbers:", error);
      return c.json({
        success: false,
        message: "L\u1ED7i khi import serial numbers h\xE0ng lo\u1EA1t"
      }, 500);
    }
  }
);
app15.post(
  "/validate",
  authenticate,
  validate(external_exports.object({
    serial_numbers: external_exports.array(external_exports.string()),
    product_id: external_exports.number().optional()
  })),
  async (c) => {
    try {
      ...
      for (const serialNumber of serial_numbers) {
        ...
        if (!isValidSerialFormat(serialNumber)) {
          ...
          result.errors.push("\u0110\u1ECBnh d\u1EA1ng kh\xF4ng h\u1EE3p l\u1EC7");
        }
        if (serialNumber.length < 3) {
          result.is_valid = false;
          result.errors.push("Serial number qu\xE1 ng\u1EAFn (t\u1ED1i thi\u1EC3u 3 k\xFD t\u1EF1)");
        }
        if (serialNumber.length > 50) {
          result.is_valid = false;
          result.errors.push("Serial number qu\xE1 d\xE0i (t\u1ED1i \u0111a 50 k\xFD t\u1EF1)");
        }
        const existing = await env.DB.prepare(
          "SELECT id FROM serial_numbers WHERE serial_number = ?"
        ...
        if (existing) {
          result.is_valid = false;
          result.errors.push("Serial number \u0111\xE3 t\u1ED3n t\u1EA1i trong h\u1EC7 th\u1ED1ng");
        }
      ...
      return c.json({
        success: false,
        message: "L\u1ED7i khi x\xE1c th\u1EF1c serial numbers"
      }, 500);
    }
  }
);
...
__name(isValidSerialFormat, "isValidSerialFormat");
var serial_numbers_default = app15;
...
app16.get("/registrations", async (c) => {
  try {
    const env = c.env;
    const page = parseInt(c.req.query("page") || "1");
    ...
    try {
      ...
      if (total > 0) {
        const results = await env.DB.prepare(`
          ...
          LIMIT ? OFFSET ?
        `).bind(limit, offset).all();
        warranties = results.results.map((row) => ({
          id: row.id,
...
app16.get("/registrations/:id", authenticate, async (c) => {
  try {
    const env = c.env;
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
...
app16.post("/registrations", authenticate, authorize(["admin", "manager", "cashier"]), validate(warrantyRegistrationCreateSchema), auditLogger, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
    const data = c.get("validatedData");
  ...
  try {
    ...
    try {
      ...
      if (totalWarranties === 0) {
        ...
        stats = {
          total_active_warranties: Math.min(totalSales, 5),
          // Based on actual sales
          expiring_soon: Math.floor(totalSales * 0.1),
          // 10% expiring soon
          expired_this_month: 0,
          pending_claims: Math.floor(totalSales * 0.05),
          // 5% have claims
          completed_claims_this_month: Math.floor(totalSales * 0.02),
          // 2% completed this month
          warranty_cost_this_month: totalSales * 5e4,
          // Average 50k per sale
      ...
      stats = {
        total_active_warranties: Math.min(totalSales, 5),
        // Based on actual sales
        expiring_soon: Math.floor(totalSales * 0.2),
        // 20% expiring soon
        expired_this_month: 0,
        pending_claims: Math.floor(totalSales * 0.1),
        // 10% have claims
        completed_claims_this_month: Math.floor(totalSales * 0.05),
        // 5% completed this month
        warranty_cost_this_month: totalSales * 75e3,
        // Average 75k per sale
        average_claim_resolution_days: 3,
...
app17.get("/", authenticate, authorize(["admin", "manager", "warranty"]), validateQuery(notificationQuerySchema), async (c) => {
  try {
    const env = c.env;
...
app17.get("/stats", authenticate, authorize(["admin", "manager", "warranty"]), async (c) => {
  try {
    const env = c.env;
    const statsQuery = `
      SELECT 
...
app17.post("/", authenticate, authorize(["admin", "manager", "warranty"]), validate(notificationCreateSchema), async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
...
app17.put("/:id", authenticate, authorize(["admin", "manager", "warranty"]), validate(notificationUpdateSchema), async (c) => {
  try {
    const env = c.env;
...
app17.post("/send-now/:id", authenticate, authorize(["admin", "manager", "warranty"]), async (c) => {
  try {
    const env = c.env;
    const notificationId = parseInt(c.req.param("id"));
    if (isNaN(notificationId)) {
...
var warranty_notifications_default = app17;
...
// src/routes/scheduled.ts
var app18 = new Hono2();
app18.post("/warranty-notifications", async (c) => {
  try {
    const env = c.env;
...
app18.get("/warranty-notifications/status", async (c) => {
  try {
    const env = c.env;
    const notificationService = new WarrantyNotificationService(env);
    const stats = await notificationService.getNotificationStats();
...
app18.post("/warranty-notifications/test", async (c) => {
  try {
    const env = c.env;
...
app18.post("/warranty-notifications/force-check", async (c) => {
  try {
    const env = c.env;
    const notificationService = new WarrantyNotificationService(env);
...
app18.post("/warranty-notifications/send-pending", async (c) => {
  try {
    const env = c.env;
    const notificationService = new WarrantyNotificationService(env);
...
app19.get("/", async (c) => {
  try {
    ...
    const sampleProducts = await c.env.DB.prepare(`
      SELECT id, name, price, stock_quantity FROM products LIMIT 3
    `).all();
    const sampleCustomers = await c.env.DB.prepare(`
      SELECT id, full_name, phone, loyalty_points FROM customers LIMIT 3
    `).all();
    return c.json({
      success: true,
      message: "100% Real D1 Cloudflare Database Connected",
      data: {
        counts: {
        ...
        samples: {
          products: sampleProducts.results || [],
          customers: sampleCustomers.results || []
        }
      }
    });
  } catch (error) {
...
// src/routes/analytics.ts
var app20 = new Hono2();
var productSuggestionsSchema = external_exports.object({
  supplier_id: external_exports.string().optional(),
  category_id: external_exports.string().optional(),
  limit: external_exports.string().default("10")
});
var inventoryForecastSchema = external_exports.object({
  category_id: external_exports.string().optional(),
  supplier_id: external_exports.string().optional(),
  forecast_days: external_exports.string().default("30")
});
app20.get("/product-suggestions", zValidator("query", productSuggestionsSchema), async (c) => {
  try {
    ...
    const suggestions = [
      {
        ...
        cost_price: 75e5,
        stock_quantity: 2,
        suggestion_reason: "S\u1EAFp h\u1EBFt h\xE0ng, b\xE1n ch\u1EA1y trong th\xE1ng",
        confidence_score: 95,
        historical_data: {
      ...
      },
      {
        ...
        cost_price: 38e5,
        stock_quantity: 5,
        suggestion_reason: "Xu h\u01B0\u1EDBng t\u0103ng, gi\xE1 t\u1ED1t t\u1EEB nh\xE0 cung c\u1EA5p",
        confidence_score: 87,
        historical_data: {
          avg_monthly_sales: 8,
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y g\u1EE3i \xFD s\u1EA3n ph\u1EA9m"
    }, 500);
  }
});
app20.get("/inventory-forecast", zValidator("query", inventoryForecastSchema), async (c) => {
  try {
    ...
    const forecasts = [
      {
        ...
        cost_impact: {
          current_value: 2095e4,
          suggested_order_value: 6285e4,
          potential_savings: 15e5
        }
      },
      {
        product_id: 2,
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y d\u1EF1 b\xE1o t\u1ED3n kho"
    }, 500);
  }
});
app20.get("/dashboard", async (c) => {
  try {
    const db = c.env.DB;
    const dashboardData = {
      summary: {
        ...
        pending_orders: 12,
        monthly_revenue: 45e7,
        monthly_growth: 12.5
      },
      stock_levels: [
        { category: "CPU", in_stock: 145, low_stock: 8, out_of_stock: 2, total_value: 85e7 },
        { category: "GPU", in_stock: 89, low_stock: 5, out_of_stock: 1, total_value: 12e8 },
        { category: "RAM", in_stock: 234, low_stock: 6, out_of_stock: 1, total_value: 32e7 },
        { category: "Storage", in_stock: 178, low_stock: 4, out_of_stock: 1, total_value: 48e7 }
      ],
      recent_activities: [
        {
          id: 1,
...
app20.get("/price-analytics", async (c) => {
  try {
    const db = c.env.DB;
      ...
      price_trends: [
        {
          ...
          current_price: 899e4,
          price_history: [
            { date: "2024-01-01", price: 92e5 },
            { date: "2024-01-08", price: 91e5 },
            { date: "2024-01-15", price: 899e4 }
          ],
          trend: "decreasing",
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y ph\xE2n t\xEDch gi\xE1"
    }, 500);
  }
});
...
init_strip_cf_connecting_ip_header();
var app21 = new Hono2();
var uploadPhotoSchema = external_exports.object({
...
app21.post("/upload", zValidator("json", uploadPhotoSchema), async (c) => {
  try {
    ...
    return c.json({
      ...
      message: "\u1EA2nh \u0111\xE3 \u0111\u01B0\u1EE3c t\u1EA3i l\xEAn th\xE0nh c\xF4ng"
    });
  } catch (error) {
    console.error("Error uploading photo:", error);
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA3i l\xEAn \u1EA3nh"
    }, 500);
  }
});
app21.get("/stock-in/:stockInId", async (c) => {
  try {
    ...
    const photos = [
      {
        ...
        description: "\u1EA2nh x\xE1c minh h\xE0ng h\xF3a",
        created_at: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y danh s\xE1ch \u1EA3nh"
    }, 500);
  }
});
app21.delete("/:id", async (c) => {
  try {
    ...
    return c.json({
      success: true,
      message: "\u1EA2nh \u0111\xE3 \u0111\u01B0\u1EE3c x\xF3a"
    });
  } catch (error) {
    console.error("Error deleting photo:", error);
    return c.json({
      success: false,
      message: "L\u1ED7i khi x\xF3a \u1EA3nh"
    }, 500);
  }
});
app21.post("/batch-upload", async (c) => {
  try {
    const body = await c.req.json();
    ...
    return c.json({
      success: true,
      data: uploadedPhotos,
      message: `\u0110\xE3 t\u1EA3i l\xEAn ${uploadedPhotos.length} \u1EA3nh th\xE0nh c\xF4ng`
    });
  } catch (error) {
    console.error("Error batch uploading photos:", error);
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA3i l\xEAn \u1EA3nh h\xE0ng lo\u1EA1t"
    }, 500);
  }
});
app21.get("/analytics", async (c) => {
  try {
    const db = c.env.DB;
    const analytics = {
      total_photos: 1247,
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi l\u1EA5y th\u1ED1ng k\xEA \u1EA3nh"
    }, 500);
  }
});
var photos_default = app21;

// src/routes/smart-serial-tracking.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var app22 = new Hono2();
app22.get("/ai-insights", authenticate, async (c) => {
  try {
    const env = c.env;
...
app22.get("/predictive-alerts", authenticate, async (c) => {
  try {
    const env = c.env;
    const alerts = await env.DB.prepare(`
      SELECT 
        'warranty_expiring_prediction' as alert_type,
        'D\u1EF1 \u0111o\xE1n b\u1EA3o h\xE0nh s\u1EAFp h\u1EBFt h\u1EA1n' as title,
        sn.serial_number,
        p.name as product_name,
        c.full_name as customer_name,
        ...
        CASE 
          ...
          ELSE 'info'
        END as severity,
        'AI d\u1EF1 \u0111o\xE1n kh\xE1ch h\xE0ng c\xF3 th\u1EC3 c\u1EA7n h\u1ED7 tr\u1EE3 b\u1EA3o h\xE0nh s\u1EDBm' as ai_reason,
        CASE 
      ...
      SELECT 
        ...
        'D\u1EF1 \u0111o\xE1n v\u1EA5n \u0111\u1EC1 ch\u1EA5t l\u01B0\u1EE3ng s\u1EA3n ph\u1EA9m' as title,
        ...
        'medium' as priority,
        'warning' as severity,
        'AI ph\xE1t hi\u1EC7n t\u1EF7 l\u1EC7 b\u1EA3o h\xE0nh cao cho s\u1EA3n ph\u1EA9m n\xE0y' as ai_reason,
        90 as confidence
      FROM products p
      WHERE p.id IN (
...
app22.get("/smart-dashboard", authenticate, async (c) => {
  try {
    const env = c.env;
...
app22.post("/auto-optimize", authenticate, authorize(["admin", "manager"]), async (c) => {
  try {
    const env = c.env;
    const optimizations = await env.DB.prepare(`
      SELECT 
        'slow_moving_inventory' as optimization_type,
        'T\u1ED3n kho ch\u1EADm lu\xE2n chuy\u1EC3n' as title,
        p.name as product_name,
        COUNT(sn.id) as serial_count,
        AVG(julianday('now') - julianday(sn.received_date)) as avg_days_in_stock,
        'Khuy\u1EBFn ngh\u1ECB: Gi\u1EA3m gi\xE1 ho\u1EB7c ch\u01B0\u01A1ng tr\xECnh khuy\u1EBFn m\xE3i' as recommendation,
        'medium' as priority
      FROM serial_numbers sn
      LEFT JOIN products p ON sn.product_id = p.id
      ...
      SELECT 
        ...
        'T\u1EF7 l\u1EC7 b\u1EA3o h\xE0nh cao' as title,
        p.name as product_name,
        COUNT(sn.id) as serial_count,
        COUNT(CASE WHEN sn.status = 'warranty_claim' THEN 1 END) as claim_count,
        'Khuy\u1EBFn ngh\u1ECB: Ki\u1EC3m tra ch\u1EA5t l\u01B0\u1EE3ng nh\xE0 cung c\u1EA5p' as recommendation,
        'high' as priority
      FROM serial_numbers sn
      LEFT JOIN products p ON sn.product_id = p.id
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA1o t\u1ED1i \u01B0u h\xF3a t\u1EF1 \u0111\u1ED9ng",
      data: null
    }, 500);
  }
});
function generateAIInsights(data) {
  ...
  if (!data || data.length === 0) {
    return ["Ch\u01B0a c\xF3 \u0111\u1EE7 d\u1EEF li\u1EC7u \u0111\u1EC3 ph\xE2n t\xEDch AI"];
  }
  ...
  if (sellThroughRate > 80) {
    insights.push(`\u{1F680} T\u1EF7 l\u1EC7 b\xE1n h\xE0ng xu\u1EA5t s\u1EAFc: ${sellThroughRate.toFixed(1)}% - Hi\u1EC7u su\u1EA5t kinh doanh r\u1EA5t t\u1ED1t!`);
  } else if (sellThroughRate < 50) {
    insights.push(`\u26A0\uFE0F T\u1EF7 l\u1EC7 b\xE1n h\xE0ng th\u1EA5p: ${sellThroughRate.toFixed(1)}% - C\u1EA7n xem x\xE9t chi\u1EBFn l\u01B0\u1EE3c marketing`);
  }
  const avgShelfTime = data.reduce((sum, item) => sum + (item.avg_shelf_time_days || 0), 0) / data.length;
  if (avgShelfTime > 90) {
    insights.push(`\u{1F4E6} Th\u1EDDi gian t\u1ED3n kho trung b\xECnh cao: ${avgShelfTime.toFixed(0)} ng\xE0y - C\u1EA7n t\u1ED1i \u01B0u h\xF3a inventory`);
  } else if (avgShelfTime < 30) {
    insights.push(`\u26A1 Lu\xE2n chuy\u1EC3n h\xE0ng h\xF3a nhanh: ${avgShelfTime.toFixed(0)} ng\xE0y - Hi\u1EC7u qu\u1EA3 qu\u1EA3n l\xFD t\u1ED1t!`);
  }
  const totalClaims = data.reduce((sum, item) => sum + (item.warranty_claims || 0), 0);
  const claimRate = totalClaims / totalSold * 100;
  if (claimRate > 5) {
    insights.push(`\u{1F527} T\u1EF7 l\u1EC7 b\u1EA3o h\xE0nh cao: ${claimRate.toFixed(1)}% - C\u1EA7n ki\u1EC3m tra ch\u1EA5t l\u01B0\u1EE3ng s\u1EA3n ph\u1EA9m`);
  } else if (claimRate < 2) {
    insights.push(`\u2705 Ch\u1EA5t l\u01B0\u1EE3ng s\u1EA3n ph\u1EA9m t\u1ED1t: T\u1EF7 l\u1EC7 b\u1EA3o h\xE0nh ch\u1EC9 ${claimRate.toFixed(1)}%`);
  }
  return insights;
}
...
function generatePredictions(data) {
  ...
  predictions.push({
    type: "sales_forecast",
    title: "D\u1EF1 b\xE1o b\xE1n h\xE0ng th\xE1ng t\u1EDBi",
    value: Math.round(avgMonthlySales * 1.1),
    // 10% growth assumption
    confidence: 0.78,
    reasoning: "D\u1EF1a tr\xEAn xu h\u01B0\u1EDBng b\xE1n h\xE0ng v\xE0 t\xEDnh m\xF9a v\u1EE5"
  });
  const totalClaims = data.reduce((sum, item) => sum + (item.warranty_claims || 0), 0);
  predictions.push({
    type: "warranty_forecast",
    title: "D\u1EF1 b\xE1o y\xEAu c\u1EA7u b\u1EA3o h\xE0nh",
    value: Math.round(totalClaims * 0.8),
    // Assuming improvement
    confidence: 0.65,
    reasoning: "D\u1EF1a tr\xEAn l\u1ECBch s\u1EED b\u1EA3o h\xE0nh v\xE0 c\u1EA3i thi\u1EC7n ch\u1EA5t l\u01B0\u1EE3ng"
  });
  return predictions;
}
...
function generateRecommendations(data) {
  ...
  if (slowMovingItems.length > 0) {
    recommendations.push({
      ...
      title: "T\u1ED1i \u01B0u h\xF3a t\u1ED3n kho",
      description: `C\xF3 ${slowMovingItems.length} nh\xF3m s\u1EA3n ph\u1EA9m lu\xE2n chuy\u1EC3n ch\u1EADm`,
      action: "Xem x\xE9t gi\u1EA3m gi\xE1 ho\u1EB7c ch\u01B0\u01A1ng tr\xECnh khuy\u1EBFn m\xE3i",
      priority: "medium",
  ...
  if (highClaimItems.length > 0) {
    recommendations.push({
      ...
      description: `C\xF3 ${highClaimItems.length} nh\xE0 cung c\u1EA5p c\xF3 t\u1EF7 l\u1EC7 b\u1EA3o h\xE0nh cao`,
      action: "\u0110\xE1nh gi\xE1 l\u1EA1i nh\xE0 cung c\u1EA5p v\xE0 ti\xEAu chu\u1EA9n ch\u1EA5t l\u01B0\u1EE3ng",
      priority: "high",
      impact: "Gi\u1EA3m 40-50% y\xEAu c\u1EA7u b\u1EA3o h\xE0nh"
    });
  }
...
function calculateAIHealthScore(metrics) {
  if (!metrics)
    return 0;
  let score = 100;
  if (metrics.active_claims > 10)
    score -= 20;
  if (metrics.urgent_warranties > 5)
    score -= 15;
  if (metrics.avg_inventory_days > 90)
    score -= 10;
  if (metrics.sold_24h > metrics.new_serials_24h)
    score += 5;
  if (metrics.active_claims < 5)
    score += 10;
  return Math.max(0, Math.min(100, score));
}
__name(calculateAIHealthScore, "calculateAIHealthScore");
var smart_serial_tracking_default = app22;

// src/routes/advanced-warranty.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var app23 = new Hono2();
app23.get("/smart-notifications", authenticate, async (c) => {
  try {
    const env = c.env;
    const notifications = await env.DB.prepare(`
      SELECT 
        'warranty_expiring' as notification_type,
        'B\u1EA3o h\xE0nh s\u1EAFp h\u1EBFt h\u1EA1n' as title,
        sn.serial_number,
        p.name as product_name,
        c.full_name as customer_name,
        c.phone as customer_phone,
        ...
        CASE 
          ...
          ELSE 'info'
        END as priority,
        'Kh\xE1ch h\xE0ng c\u1EA7n \u0111\u01B0\u1EE3c th\xF4ng b\xE1o v\u1EC1 vi\u1EC7c b\u1EA3o h\xE0nh s\u1EAFp h\u1EBFt h\u1EA1n' as message,
      ...
      SELECT 
        ...
        'B\u1EA3o h\xE0nh \u0111\xE3 h\u1EBFt h\u1EA1n' as title,
        sn.serial_number,
        p.name as product_name,
        c.full_name as customer_name,
        c.phone as customer_phone,
        ...
        sn.warranty_end_date,
        'expired' as priority,
        'Kh\xE1ch h\xE0ng c\xF3 th\u1EC3 quan t\xE2m \u0111\u1EBFn d\u1ECBch v\u1EE5 m\u1EDF r\u1ED9ng b\u1EA3o h\xE0nh' as message,
      ...
      SELECT 
        ...
        'Theo d\xF5i y\xEAu c\u1EA7u b\u1EA3o h\xE0nh' as title,
        sn.serial_number,
        p.name as product_name,
        c.full_name as customer_name,
        c.phone as customer_phone,
        ...
        CASE 
          ...
          ELSE 'info'
        END as priority,
        'Y\xEAu c\u1EA7u b\u1EA3o h\xE0nh c\u1EA7n \u0111\u01B0\u1EE3c theo d\xF5i v\xE0 c\u1EADp nh\u1EADt' as message,
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA3i th\xF4ng b\xE1o th\xF4ng minh",
      data: null
    }, 500);
  }
});
  ...
  async (c) => {
    try {
      ...
      for (const notification of notifications) {
        try {
          ...
          `).bind(
            ...
            `Th\xF4ng b\xE1o: B\u1EA3o h\xE0nh s\u1EA3n ph\u1EA9m ${notification.product_name} (SN: ${notification.serial_number}) s\u1EBD h\u1EBFt h\u1EA1n v\xE0o ${notification.warranty_end_date}. Li\xEAn h\u1EC7 c\u1EEDa h\xE0ng \u0111\u1EC3 \u0111\u01B0\u1EE3c h\u1ED7 tr\u1EE3.`,
            user.sub
          ).run();
          results.push({
      ...
      return c.json({
        ...
        message: `\u0110\xE3 g\u1EEDi ${sentCount}/${notifications.length} th\xF4ng b\xE1o b\u1EA3o h\xE0nh`
      });
    } catch (error) {
      console.error("\u274C Error auto-sending notifications:", error);
      return c.json({
        success: false,
        message: "L\u1ED7i khi g\u1EEDi th\xF4ng b\xE1o t\u1EF1 \u0111\u1ED9ng",
        data: null
      }, 500);
    }
  }
);
app23.get("/analytics", authenticate, async (c) => {
  try {
    const env = c.env;
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA3i ph\xE2n t\xEDch b\u1EA3o h\xE0nh",
      data: null
    }, 500);
  }
});
  ...
  async (c) => {
    try {
      ...
      const schema = external_exports.object({
        claim_id: external_exports.number().int().positive(),
        ai_assessment: external_exports.boolean().default(true),
        auto_approve_threshold: external_exports.number().min(0).max(100).default(80)
      });
      ...
      if (!claim) {
        return c.json({
          success: false,
          message: "Kh\xF4ng t\xECm th\u1EA5y y\xEAu c\u1EA7u b\u1EA3o h\xE0nh",
          data: null
        }, 404);
      }
      ...
      return c.json({
        success: false,
        message: "L\u1ED7i khi x\u1EED l\xFD y\xEAu c\u1EA7u b\u1EA3o h\xE0nh th\xF4ng minh",
        data: null
      }, 500);
    }
  }
);
function generateWarrantyInsights(overview, trends) {
  const insights = [];
  if (!overview)
    return insights;
  const claimRate = overview.total_claims / overview.total_warranties * 100;
  if (claimRate > 15) {
    insights.push(`\u26A0\uFE0F T\u1EF7 l\u1EC7 y\xEAu c\u1EA7u b\u1EA3o h\xE0nh cao: ${claimRate.toFixed(1)}% - C\u1EA7n ki\u1EC3m tra ch\u1EA5t l\u01B0\u1EE3ng s\u1EA3n ph\u1EA9m`);
  } else if (claimRate < 5) {
    insights.push(`\u2705 T\u1EF7 l\u1EC7 y\xEAu c\u1EA7u b\u1EA3o h\xE0nh th\u1EA5p: ${claimRate.toFixed(1)}% - Ch\u1EA5t l\u01B0\u1EE3ng s\u1EA3n ph\u1EA9m t\u1ED1t`);
  }
  if (overview.avg_resolution_days > 7) {
    insights.push(`\u{1F550} Th\u1EDDi gian x\u1EED l\xFD trung b\xECnh: ${overview.avg_resolution_days.toFixed(1)} ng\xE0y - C\u1EA7n c\u1EA3i thi\u1EC7n quy tr\xECnh`);
  } else if (overview.avg_resolution_days < 3) {
    insights.push(`\u26A1 X\u1EED l\xFD nhanh ch\xF3ng: ${overview.avg_resolution_days.toFixed(1)} ng\xE0y - D\u1ECBch v\u1EE5 xu\u1EA5t s\u1EAFc!`);
  }
  if (overview.avg_claim_cost > 1e6) {
    insights.push(`\u{1F4B0} Chi ph\xED b\u1EA3o h\xE0nh trung b\xECnh cao: ${(overview.avg_claim_cost / 1e6).toFixed(1)}M VN\u0110`);
  }
  if (trends && trends.length >= 2) {
    ...
    if (latestMonth && previousMonth) {
      ...
      if (claimGrowth > 20) {
        insights.push(`\u{1F4C8} Y\xEAu c\u1EA7u b\u1EA3o h\xE0nh t\u0103ng ${claimGrowth.toFixed(1)}% so v\u1EDBi th\xE1ng tr\u01B0\u1EDBc`);
      } else if (claimGrowth < -20) {
        insights.push(`\u{1F4C9} Y\xEAu c\u1EA7u b\u1EA3o h\xE0nh gi\u1EA3m ${Math.abs(claimGrowth).toFixed(1)}% so v\u1EDBi th\xE1ng tr\u01B0\u1EDBc`);
      }
    }
  }
  return insights;
}
__name(generateWarrantyInsights, "generateWarrantyInsights");
function performAIClaimAssessment(claim) {
  let confidence = 50;
  ...
  if (reportedDate <= warrantyEndDate) {
    confidence += 30;
    reasoning += "Trong th\u1EDDi h\u1EA1n b\u1EA3o h\xE0nh. ";
  } else {
    confidence -= 40;
    reasoning += "Ngo\xE0i th\u1EDDi h\u1EA1n b\u1EA3o h\xE0nh. ";
  }
  const validClaimTypes = ["defective", "malfunction", "damage"];
  if (validClaimTypes.includes(claim.claim_type)) {
    confidence += 20;
    reasoning += "Lo\u1EA1i y\xEAu c\u1EA7u h\u1EE3p l\u1EC7. ";
  }
  if (claim.estimated_cost && claim.estimated_cost < 5e5) {
    confidence += 15;
    reasoning += "Chi ph\xED \u01B0\u1EDBc t\xEDnh h\u1EE3p l\xFD. ";
  } else if (claim.estimated_cost > 2e6) {
    confidence -= 20;
    reasoning += "Chi ph\xED \u01B0\u1EDBc t\xEDnh cao. ";
  }
  if (claim.description && claim.description.length > 50) {
    confidence += 10;
    reasoning += "M\xF4 t\u1EA3 chi ti\u1EBFt. ";
  }
  if (confidence >= 80) {
  ...
  return {
    ...
    factors: {
      ...
      cost_reasonable: claim.estimated_cost < 5e5,
      description_adequate: claim.description && claim.description.length > 50
    }
  };
}
__name(performAIClaimAssessment, "performAIClaimAssessment");
var advanced_warranty_default = app23;

// src/routes/realtime-notifications.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
var app24 = new Hono2();
app24.get("/stream", authenticate, async (c) => {
  const user = getUser(c);
      ...
      const interval = setInterval(async () => {
        try {
          ...
          const notifications = await env.DB.prepare(`
            ...
            SELECT 
              'low_stock' as type,
              'S\u1EA3n ph\u1EA9m s\u1EAFp h\u1EBFt h\xE0ng' as title,
              '' as serial_number,
              p.name as product_name,
              '' as customer_name,
            ...
            SELECT 
              ...
              'Y\xEAu c\u1EA7u b\u1EA3o h\xE0nh m\u1EDBi' as title,
              sn.serial_number,
              p.name as product_name,
              c.full_name as customer_name,
              wc.reported_date as warranty_end_date,
...
app24.get("/latest", authenticate, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
    const notifications = await env.DB.prepare(`
      SELECT 
        'warranty_expiring' as type,
        'B\u1EA3o h\xE0nh s\u1EAFp h\u1EBFt h\u1EA1n' as title,
        'S\u1EA3n ph\u1EA9m ' || p.name || ' (SN: ' || sn.serial_number || ') c\u1EE7a kh\xE1ch h\xE0ng ' || c.full_name || ' s\u1EBD h\u1EBFt b\u1EA3o h\xE0nh v\xE0o ' || sn.warranty_end_date as message,
      ...
      SELECT 
        'low_stock' as type,
        'S\u1EA3n ph\u1EA9m s\u1EAFp h\u1EBFt h\xE0ng' as title,
        'S\u1EA3n ph\u1EA9m ' || p.name || ' ch\u1EC9 c\xF2n ' || p.stock_quantity || ' trong kho' as message,
        datetime('now') as created_at,
        CASE 
      ...
      SELECT 
        'new_sale' as type,
        '\u0110\u01A1n h\xE0ng m\u1EDBi' as title,
        '\u0110\u01A1n h\xE0ng #' || s.receipt_number || ' tr\u1ECB gi\xE1 ' || printf('%.0f', s.total_amount) || ' VN\u0110' as message,
        s.created_at,
        'success' as severity,
        'sales' as category
      FROM sales s
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi t\u1EA3i th\xF4ng b\xE1o",
      data: null
    }, 500);
  }
});
app24.post("/mark-read", authenticate, async (c) => {
  try {
    const env = c.env;
    const user = getUser(c);
    ...
    return c.json({
      success: false,
      message: "L\u1ED7i khi \u0111\xE1nh d\u1EA5u th\xF4ng b\xE1o \u0111\xE3 \u0111\u1ECDc",
      data: null
    }, 500);
  }
});
var realtime_notifications_default = app24;

// src/index.ts
var app25 = new Hono2();
app25.use("*", corsSecurity);
app25.use("*", validateEnvironment);
app25.get("/test-product/:id", async (c) => {
  try {
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({
...
app25.use("*", accessLogger);
app25.use("*", securityHeaders);
app25.use("*", sqlInjectionProtection);
var api = new Hono2();
api.get("/health", (c) => {
  return c.json({
    success: true,
...
api.get("/customers", async (c) => {
  try {
    ...
    if (search && search.trim()) {
      ...
      params2.push(`%${search}%`, `%${search}%`, `%${search}%`);
    }
    const whereClause2 = conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
    const countQuery = `SELECT COUNT(*) as total FROM customers ${whereClause2}`;
    const countResult = await c.env.DB.prepare(countQuery).bind(...params2).first();
    const total = countResult?.total || 0;
    const customersQuery = `
      SELECT
        id, full_name, phone, email, address, loyalty_points, notes, created_at
      FROM customers
      ${whereClause2}
      ORDER BY created_at DESC
      LIMIT ? OFFSET ?
    `;
    const customersResult = await c.env.DB.prepare(customersQuery).bind(...params2, limit, offset).all();
    const totalPages = Math.ceil(total / limit);
    return c.json({
      success: true,
      data: {
    ...
    return c.json({
      success: false,
      data: null,
      message: "L\u1ED7i khi l\u1EA5y danh s\xE1ch kh\xE1ch h\xE0ng: " + (error instanceof Error ? error.message : String(error))
    }, 500);
  }
});
...
api.route("/smart-serial-tracking", smart_serial_tracking_default);
api.use("/advanced-warranty/*", rateLimit("default"));
api.use("/advanced-warranty/*", authenticate);
api.route("/advanced-warranty", advanced_warranty_default);
api.use("/realtime-notifications/*", rateLimit("default"));
api.use("/realtime-notifications/*", authenticate);
api.route("/realtime-notifications", realtime_notifications_default);
...
app25.route("/", websocket_default);
app25.route("/api/v1", api);
app25.get("/", (c) => c.text("SmartPOS API - S\u1EED d\u1EE5ng endpoint /api/v1 \u0111\u1EC3 truy c\u1EADp API"));
app25.notFound((c) => {
  return c.json({
    success: false,
